...
function calcAll(f) {
  ...
  const shinyoBairitu = n(f.shinyoBairitu);
  ...
  return {
    marketCap, ev, ebitda, businessProfit, financialCost, shinyoBairitu,
...
const EMPTY_F = {
  ...
  fixedLiabilities:"", receivables:"", inventory:"", tangibleFixedAssets:"",
  quickAssets:"", interestExpense:"", depreciation:"", ebitda:"", netDebt:"",
  dividend:"", epsGrowth:"", shinyoBairitu:""
};

const INITIAL_PORTFOLIO = [
  { id:1, ticker:"7203", name:"トヨタ自動車", sector:"自動車", qty:100, avgCost:2850, currentPrice:3124,
    financials:{ price:"3124", shares:"14430000", eps:"306", bps:"2841", sales:"4390000000000", grossProfit:"900000000000", sgaExpense:"550000000000", laborCost:"200000000000", operatingProfit:"350000000000", interestIncome:"10000000000", ordinaryProfit:"360000000000", netProfit:"400000000000", totalAssets:"9000000000000", operatingAssets:"7000000000000", equity:"3700000000000", currentAssets:"5000000000000", fixedAssets:"4000000000000", currentLiabilities:"3000000000000", fixedLiabilities:"2300000000000", receivables:"800000000000", inventory:"400000000000", tangibleFixedAssets:"2000000000000", quickAssets:"2300000000000", interestExpense:"50000000000", depreciation:"100000000000", ebitda:"450000000000", netDebt:"1500000000000", dividend:"75", epsGrowth:"8", shinyoBairitu:"2.1" },
    irList:[] },
  { id:2, ticker:"6758", name:"ソニーグループ", sector:"電機", qty:50, avgCost:12400, currentPrice:13250,
    financials:{ price:"13250", shares:"1190000", eps:"720", bps:"5760", sales:"13000000000000", grossProfit:"4200000000000", sgaExpense:"2800000000000", laborCost:"", operatingProfit:"1200000000000", interestIncome:"50000000000", ordinaryProfit:"1250000000000", netProfit:"900000000000", totalAssets:"25000000000000", operatingAssets:"20000000000000", equity:"6800000000000", currentAssets:"12000000000000", fixedAssets:"13000000000000", currentLiabilities:"8000000000000", fixedLiabilities:"10000000000000", receivables:"2000000000000", inventory:"800000000000", tangibleFixedAssets:"3000000000000", quickAssets:"10000000000000", interestExpense:"100000000000", depreciation:"400000000000", ebitda:"1600000000000", netDebt:"2000000000000", dividend:"95", epsGrowth:"10", shinyoBairitu:"1.4" },
    irList:[] },
  { id:3, ticker:"9984", name:"ソフトバンクG", sector:"通信", qty:200, avgCost:7200, currentPrice:6850,
    financials:{ price:"6850", shares:"2110000", eps:"", bps:"3800", sales:"6000000000000", grossProfit:"", sgaExpense:"", laborCost:"", operatingProfit:"500000000000", interestIncome:"", ordinaryProfit:"", netProfit:"-800000000000", totalAssets:"50000000000000", operatingAssets:"", equity:"8000000000000", currentAssets:"", fixedAssets:"", currentLiabilities:"", fixedLiabilities:"", receivables:"", inventory:"", tangibleFixedAssets:"", quickAssets:"", interestExpense:"", depreciation:"", ebitda:"900000000000", netDebt:"18000000000000", dividend:"", epsGrowth:"", shinyoBairitu:"3.2" },
    irList:[] },
];
...
const INPUT_FIELDS = [
  [["株価（円）","price"],["発行済株式数（千株）","shares"],["EPS（円）","eps"]],
  [["BPS（円）","bps"],["1株配当（円）","dividend"],["EPS成長率（来期予想）%","epsGrowth"]],
  [["売上高（円）","sales"],["売上総利益（円）","grossProfit"],["販管費（円）","sgaExpense"]],
  [["人件費（円）","laborCost"],["営業利益（円）","operatingProfit"],["受取利息・配当（円）","interestIncome"]],
  [["経常利益（円）","ordinaryProfit"],["当期純利益（円）","netProfit"],["総資本・総資産（円）","totalAssets"]],
  [["経営資本（円）","operatingAssets"],["自己資本・純資産（円）","equity"],["流動資産（円）","currentAssets"]],
  [["固定資産（円）","fixedAssets"],["流動負債（円）","currentLiabilities"],["固定負債（円）","fixedLiabilities"]],
  [["売上債権（円）","receivables"],["棚卸資産（円）","inventory"],["有形固定資産（円）","tangibleFixedAssets"]],
  [["当座資産（円）","quickAssets"],["支払利息（円）","interestExpense"],["減価償却費（円）","depreciation"]],
  [["EBITDA（円）","ebitda"],["純有利子負債（円）","netDebt"],["信用倍率（倍）","shinyoBairitu"]],
];
...
export default function App() {
  ...
  const handleAddStock = () => {
    ...
    const newStock = { id:Date.now(), ticker, name, sector:sector||"—", qty:+qty, avgCost:+avgCost, currentPrice:+currentPrice, financials:{...EMPTY_F,price:currentPrice}, irList:[] };
  ...
  const simData = useCallback(() => {
    ...
    return Array.from({length:years+1},(_,y)=>{
      ...
      const projEps = eps>0?eps*gf:projOP/(shares*1000);
      ...
      const priceBull = projEps>0?Math.round(projEps*Math.pow(1+g*1.6/g,y)*tPer*1.2):null;
      const mCapFromEv = tEvEb!=null?(projEbitda*tEvEb-netDebt):null;
      const priceFromEv = (mCapFromEv!=null&&shares>0)?Math.round(mCapFromEv/(shares*1000)):null;
      const divCum = simParams.reinvest?Math.round(price*(Math.pow(1+divRate,y)-1)):Math.round(price*divRate*y);
      return { year:y===0?"現在":`${y}年後`, base株価:priceBase, bear株価:priceBear, bull株価:priceBull, EV推定株価:priceFromEv, 配当累計:divCum, 売上:Math.round(projSales), 営業利益:Math.round(projOP), EPS:parseFloat(projEps.toFixed(2)) };
    });
  },[selected,simParams]);
      ...
      <main style={S.main}>
        ...
        {tab==="portfolio"&&(
          <div>
            ...
            {portfolio.length===0&&<div style={{...S.card,textAlign:"center",color:"#475569",padding:40}}>銘柄がありません。「＋ 銘柄追加」から追加してください。</div>}
          </div>
        )}
          ...
          <div>
            ...
            {selected&&(
              <>
                ...
                {detailTab==="overview"&&(
                  <div style={S.card}>
                    <div style={{color:"#64748b",fontSize:12,marginBottom:12}}>主要指標サマリー</div>
                    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(145px,1fr))",gap:10}}>
                      ...
                      <MBox label="PSR" value={c.psr?x(c.psr):"—"} color={c.psr&&c.psr<2?"#4ade80":"#94a3b8"}/>
                      ...
                      <MBox label="営業利益率" value={c.operatingMargin?pct(c.operatingMargin):"—"} color={c.operatingMargin&&c.operatingMargin>0.10?"#4ade80":"#94a3b8"}/>
                      <MBox label="自己資本比率" value={c.equityRatio?pct(c.equityRatio):"—"} color={c.equityRatio&&c.equityRatio>0.40?"#4ade80":"#94a3b8"}/>
                      <MBox label="信用倍率" value={f.shinyoBairitu?f.shinyoBairitu+"倍":"—"} color={n(f.shinyoBairitu)&&n(f.shinyoBairitu)>3?"#f87171":n(f.shinyoBairitu)&&n(f.shinyoBairitu)<1?"#4ade80":"#94a3b8"} hint="1倍未満は売り優勢"/>
                      <MBox label="時価総額" value={c.marketCap?fmtM(c.marketCap):"—"} color="#e2e8f0"/>
                      <MBox label="EV（企業価値）" value={c.ev?fmtM(c.ev):"—"} color="#e2e8f0"/>
                ...
                {detailTab==="profitability"&&(
                  <div>
                    <Section title="① 資本利益率">
                      <MBox label="ROA（総資本経常利益率）" value={c.roa?pct(c.roa):"—"} color={c.roa&&c.roa>0.05?"#4ade80":"#94a3b8"} hint="5%超で優良"/>
                      <MBox label="経営資本営業利益率（ROIC）" value={c.roic?pct(c.roic):"—"} color={c.roic&&c.roic>0.08?"#4ade80":"#94a3b8"}/>
                      <MBox label="ROE（自己資本利益率）" value={c.roe?pct(c.roe):"—"} color={c.roe&&c.roe>0.15?"#4ade80":"#94a3b8"} hint="15%超で優良"/>
                    </Section>
                    <Section title="② 売上高利益率">
                      <MBox label="粗利率（売上高総利益率）" value={c.grossMargin?pct(c.grossMargin):"—"} color={c.grossMargin&&c.grossMargin>0.40?"#4ade80":"#94a3b8"}/>
                    ...
                    {c.grossMargin&&c.operatingMargin&&(
                      <div style={S.card}>
                        <div style={{color:"#94a3b8",fontSize:13,marginBottom:12}}>利益率比較グラフ</div>
                        <ResponsiveContainer width="100%" height={200}>
                          <BarChart data={[
                            {name:"粗利率",value:parseFloat((c.grossMargin*100).toFixed(1))},
                            {name:"営業利益率",value:parseFloat((c.operatingMargin*100).toFixed(1))},
                            {name:"経常利益率",value:c.ordinaryMargin?parseFloat((c.ordinaryMargin*100).toFixed(1)):0},
                ...
                {detailTab==="input"&&(
                  <div>
                    ...
                    {INPUT_FIELDS.map((row,i)=>(
                      <div key={i} style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:12,marginBottom:12}}>
                        {row.map(([label,key])=>(
                          <FInput key={key} label={label} value={f[key]||""} onChange={v=>updateF(selected.id,key,v)}/>
                        ))}
                      </div>
                    ))}
                  </div>
                  ...
                  <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                    <thead>
                      <tr style={{borderBottom:"1px solid #1e293b"}}>
                        <th style={{textAlign:"left",padding:"8px 12px",color:"#475569",fontSize:11,minWidth:170}}>指標</th>
                    ...
                    <tbody>
                      {[
                        ...
                        ["ICR（インタレストカバレッジ）",h=>calcAll(h.financials).icr,v=>v?v.toFixed(2)+"倍":"—",v=>v&&v>3?"#4ade80":"#94a3b8"],
                        ["信用倍率",h=>h.financials.shinyoBairitu,v=>v?v+"倍":"—",v=>n(v)>3?"#f87171":"#94a3b8"],
                        ["時価総額",h=>calcAll(h.financials).marketCap,v=>v?fmtM(v):"—",()=>"#e2e8f0"],
                        ["EBITDA",h=>calcAll(h.financials).ebitda,v=>v?fmtM(v):"—",()=>"#60a5fa"],
              ...
              <>
                ...
                <div style={S.card}>
                  ...
                  <div style={{color:"#475569",fontSize:12,marginBottom:12}}>
                    強気: 成長率{Math.round(+simParams.growthRate*1.6)}% / 基本: {simParams.growthRate}% / 弱気: {Math.round(+simParams.growthRate*0.4)}% → 目標PER {simParams.targetPer}倍
                  </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
                  ...
                  <div style={S.card}>
                    <div style={{color:"#94a3b8",fontWeight:700,marginBottom:4}}>📊 売上・営業利益推移予測</div>
                      ...
                      <tbody>
                        {simData().map((row,i)=>(
                          <tr key={i} style={{borderBottom:"1px solid #1e293b",background:i===0?"#111827":"transparent"}}>
                            <td style={{padding:"8px 10px",color:"#94a3b8",whiteSpace:"nowrap",fontWeight:i===0?700:400}}>{row.year}</td>
